...
public class SeeImageViewHelp {
    ...
    private boolean doubleClickShow = false;//双击效果 true = 初始化  false = 3倍放大
    ...
    public void moveUP(final ImageView view) {
        ...
        if (dwidth <= vwidth && dheight <= vheight) {//缩小 返回居中
            ...
            backEdging(new ValueAnimator.AnimatorUpdateListener() {
                           @Override
                           public void onAnimationUpdate(ValueAnimator animation) {
                               if (drawMode == 1)
                                   drawMode = 0;
                           }
                       }, view, vwidth, vheight, dwidth, dheight
    ...
    //放大后 靠边
    private void backEdging(final ValueAnimator.AnimatorUpdateListener callback, final ImageView view, float vwidth, float vheight, float dwidth, float dheight
        ...
        if (x1 != translationX) {
            ...
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    if (callback != null)
                        callback.onAnimationUpdate(animation);
                }
            });
            animator.start();
            view.setTag(R.id.Animation, animator);
